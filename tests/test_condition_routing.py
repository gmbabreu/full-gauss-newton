...
class ConditionRoutingTest(unittest.TestCase):
    def setUp(self):
        ...
        definitions = ast.Module(body=[nodes['flatten_condition'],
                                      nodes['run_condition_diagnostics']], type_ignores=[])
        self.calls = []
        ...
        flags = SimpleNamespace(inner_loop_wd=0., spectrum_top_k=2,
            ...
            condition_eigen_residual_tol=1e-4, condition_trace_probes=4,
    ...
    def run_controller(self, **kwargs):
        ...
        jax.effects_barrier()
        self.assertEqual(result['condition/gn_products'], len(self.calls))
        return result
    ...
    def test_non_cg_reuses_spectrum_maximum_without_damped_work(self):
        ...
        self.assertAlmostEqual(result['condition/G/lambda_max_est'], 10., delta=.01)
        self.assertFalse(any(k.startswith('condition/A/') for k in result))
        self.assertEqual(result['condition/gn_products'], result['spectrum/G/gn_products'] + 4)
    ...
    def test_cg_damped_endpoints_and_compiled_reuse_with_new_diagonal(self):
        for scale in (1., 2.):
            ...
            self.assertTrue(result['condition/A/resolved'], result)
            np.testing.assert_allclose(
                [result['condition/A/lambda_min_est'], result['condition/A/lambda_max_est']],
                [expected.min(), expected.max()], rtol=1e-4)
            self.assertAlmostEqual(result['condition/A/condition_est'],
                                   expected.max()/expected.min(), delta=1e-3)
            self.assertIn('condition/A_preconditioned/lambda_max_est', result)
    ...
    def test_failed_spectrum_falls_back_for_g_maximum(self):
        ...
        self.assertTrue(result['condition/G/resolved'])
        self.assertAlmostEqual(result['condition/G/lambda_max_est'], 10., delta=.01)
